...
def plot_roc_curve(fpr, tpr, roc_auc, model):
    ...
    ax.plot(fpr, tpr, label='ROC curve (area = %0.2f)' % roc_auc, linewidth=4)
    ax.plot([0, 1], [0, 1], 'k--', linewidth=4)
    ax.set_xlim([-0.05, 1.0])
    ax.set_ylim([-0.05, 1.05])
    ax.set_xlabel('False Positive Rate', fontsize=18)
    ax.set_ylabel('True Positive Rate', fontsize=18)
    ax.set_title(f'ROC for language error predictions - {model}', fontsize=18)
    ax.legend(loc="lower right")
    plt.show()
    

def plot_confusion_matrix(cmtrx, model):
